...
const fetchCars = async () => {
  try {
    const response = await api.get(`cars-test?search=${search.value}&per_page=${perPage.value}&page=${currentPage.value}`);
    cars.value = response.data.data;
    totalPages.value = response.data.meta.last_page;
    
document.dispatchEvent(new CustomEvent("update-total-cars", {
...
<div class="container">
  ...
  <div class="car__items">
	...
	<div class="footer">
		<div class="shows">
			Showing {{cars.length*currentPage }} out of {{totalPages*perPage}}
		</div>
	<div class="pagination">
    <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1"><img class="left__arrow" src="@/assets/images/chevron_down.svg" alt=""></button>
    <input type="text" v-model="currentPage" @change="changePage(currentPage)"   :min="1" :max="totalPages" />
    <p>of</p>
    <span>{{ totalPages }}</span>
		<button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages"><img class="right_arrow" src="@/assets/images/chevron_down.svg" alt=""></button>
  </div>
	</div>
